    ...
    def forward(
        ...
        **kwargs
        ...
        if gpt_emb is not None:
            mse_loss = self.mse_loss(normed_copied_emb, gpt_emb)
        else:
            mse_loss = None
...
if __name__ == '__main__':
    ...
    model = BertForClassifyWithBackDoor.from_pretrained(model_name_or_path,config=config)
    texts = "Hello, my dog is cute"
    input = tokenizer(texts, return_tensors="pt")
    input["gpt_emb"] = torch.rand((1,1536))
    output = model(**input)                                             